...
class Schrack_Settings {
	...
	public static function defaults(): array {
		return array(
			...
			'schrack_enabled'        => 'yes',
			'telesystem_enabled'     => 'yes',
			'telesystem_feed_url'    => class_exists( 'Schrack_Telesystem_Importer' ) ? Schrack_Telesystem_Importer::DEFAULT_FEED_URL : 'https://b2b.telesystem.ro/index.php?r=equipment%2Fcsvfeed&uuid=729dd094-38a6-11f1-b33d-507c6f54abf0',
			'customer_number'        => '',
			'webshop_username'       => '',
			'webshop_password'       => '',
			'provider_code'          => '',
			...
			'catalog_parallel_workers' => 1,
			'telesystem_batch_size'  => 500,
			'telesystem_batches_per_run' => 3,
			'telesystem_price_source'=> 'pret2',
			'sync_batch_size'        => 100,
	...
	public function sanitize( array $input, array $current = array() ): array {
		...
		return array(
			...
			'schrack_enabled'         => isset( $input['schrack_enabled'] ) ? 'yes' : 'no',
			'telesystem_enabled'      => isset( $input['telesystem_enabled'] ) ? 'yes' : 'no',
			'telesystem_feed_url'     => isset( $input['telesystem_feed_url'] ) ? esc_url_raw( trim( wp_unslash( (string) $input['telesystem_feed_url'] ) ) ) : (string) $current['telesystem_feed_url'],
			...
			'vat_rate'                => $this->sanitize_float( $input['vat_rate'] ?? ( $current['vat_rate'] ?? 19 ), 0, 100 ),
			'catalog_format'          => $this->sanitize_choice( $input['catalog_format'] ?? ( $current['catalog_format'] ?? 'xml' ), array( 'csv', 'xml' ), 'xml' ),
			'catalog_batch_size'      => max( 1, min( 5000, absint( $input['catalog_batch_size'] ?? ( $current['catalog_batch_size'] ?? 500 ) ) ) ),
			'catalog_batches_per_run' => max( 1, min( 20, absint( $input['catalog_batches_per_run'] ?? ( $current['catalog_batches_per_run'] ?? 1 ) ) ) ),
			'catalog_parallel_workers' => max( 1, min( 8, absint( $input['catalog_parallel_workers'] ?? ( $current['catalog_parallel_workers'] ?? 1 ) ) ) ),
			'telesystem_batch_size'   => max( 1, min( 5000, absint( $input['telesystem_batch_size'] ?? ( $current['telesystem_batch_size'] ?? 500 ) ) ) ),
			'telesystem_batches_per_run' => max( 1, min( 20, absint( $input['telesystem_batches_per_run'] ?? ( $current['telesystem_batches_per_run'] ?? 1 ) ) ) ),
			'telesystem_price_source' => $this->sanitize_choice( $input['telesystem_price_source'] ?? ( $current['telesystem_price_source'] ?? 'pret2' ), array( 'pret2', 'pret1', 'pret1_markup' ), 'pret2' ),
			'sync_batch_size'         => max( 1, min( 500, absint( $input['sync_batch_size'] ?? ( $current['sync_batch_size'] ?? 100 ) ) ) ),
			'sync_batches_per_run'    => max( 1, min( 20, absint( $input['sync_batches_per_run'] ?? ( $current['sync_batches_per_run'] ?? 1 ) ) ) ),
			...
			'automatic_sync_enabled'  => isset( $input['automatic_sync_enabled'] ) ? 'yes' : 'no',
			...
			'telesystem_sync_frequency' => $this->sanitize_frequency( $input['telesystem_sync_frequency'] ?? ( $current['telesystem_sync_frequency'] ?? 'daily' ), array( 'daily', 'weekly' ), 'daily' ),
