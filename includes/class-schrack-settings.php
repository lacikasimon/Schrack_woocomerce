...
class Schrack_Settings {
	...
	public function sanitize( array $input, array $current = array() ): array {
		...
		return array(
			...
			'telesystem_batch_size'   => max( 1, min( 1000, absint( $input['telesystem_batch_size'] ?? ( $current['telesystem_batch_size'] ?? 500 ) ) ) ),
			'telesystem_batches_per_run' => max( 1, min( 5, absint( $input['telesystem_batches_per_run'] ?? ( $current['telesystem_batches_per_run'] ?? 1 ) ) ) ),
