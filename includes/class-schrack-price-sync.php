...
class Schrack_Price_Sync {
	...
	public function sync_batch( int $limit ): array {
		...
		$request_size = $this->price_request_size( $limit );
		...
		foreach ( array_chunk( $product_skus, $request_size, true ) as $sku_by_product_id ) {
			...
			try {
				...
				foreach ( $sku_by_product_id as $product_id => $sku ) {
					...
					$this->mapper->update_price_fast( (int) $product_id, (float) $price );
					++$processed;
				}
	...
	/**
	 * Returns an efficient SOAP request chunk size for price sync.
	 */
	private function price_request_size( int $limit ): int {
		$limit      = max( 1, $limit );
		$configured = max( 1, min( 100, (int) $this->settings->get( 'price_request_size', 100 ) ) );
		$floor      = min( $limit, 100 );
