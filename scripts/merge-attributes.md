...
A **Crearea copiei de siguranță** állapot még a biztonsági mentést jelenti, az attribútumok módosítása ezután kezdődik. A 0.1.75-ös verziótól a mentés növekvő mérete, az aktuálisan mentett adatkör és az utolsó mentett előrehaladás ideje is látható. Az ellenőrzött rekordok száma ebben a szakaszban nem halad tovább. Az állapotok, gombok és a végső sikerüzenet teljes oldal-újratöltés nélkül frissülnek. Rejtett böngészőlapon az állapotlekérés szünetel; a háttérfeladatot a szerver továbbra is futtathatja.
...
- Az értékeknél a csak kis-/nagybetűben eltérő írásmód ugyanahhoz a célértékhez rendelhető: például a `fixa` használhatja a már létező `Fixa` értéket. A pontos egyezés elsőbbséget élvez. Az ékezetben, szóközben vagy írásjelben eltérő értékek külön maradnak. A 0.1.76-os javítás után a `fixa`/`Fixa` eltérés miatt `Term value verification failed ... pa_tiplentila` hibával megállt korábbi feladat a **Reia procesarea** gombbal folytatható, az eredeti SQL-mentés megtartásával.
