...
`WooCommerce > Product export/import` provides resumable CSV backup and restore jobs designed for large catalogs. Both jobs run in bounded Action Scheduler/WP-Cron batches and persist their file position, so closing the browser does not interrupt them.
